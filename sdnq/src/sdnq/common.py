...
if os.environ.get("SDNQ_USE_TRITON_MM", None) is None:
    # Enable Triton MM for:
    # - RDNA2 (no torch._int_mm support)
    # - ZLUDA (same)
    # - Modern CUDA GPUs with SM >= 89 (4090/5090) where Triton autotuned kernel is faster
    is_modern_cuda = False
    if devices.backend == "cuda":
        try:
            sm = torch.cuda.get_device_capability(devices.device)
            is_modern_cuda = sm >= (8, 9)  # SM89 = 4090, SM100+ = 5090
        except Exception:
            pass
    use_triton_mm = bool(is_rdna2 or devices.backend == "zluda" or is_modern_cuda)
else:
