...
def fp8_matmul_backward_ckpt(
    ...
    if do_grad_input:
        grad_input = fp8_matmul_dynamic(grad_output, dequantize_symmetric(weight, scale), svd_up=svd_up, svd_down=svd_down, output_shape=input.shape, do_input_reshape=False, use_sr=True)
    if do_grad_weight:
